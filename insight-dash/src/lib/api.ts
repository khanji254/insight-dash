...
export const authApi = {
  ...
  verifyOtp: (email: string, code: string) =>
    post<ApiResponse<{ message: string }>>("/auth/verify-otp", { email, code }),
...
export const adminApi = {
  getPendingUsers: () =>
    get<ApiResponse<{ users: AdminUser[] }>>("/admin/users/pending"),
  getAllUsers: () =>
    get<ApiResponse<{ users: AdminUser[] }>>("/admin/users"),
  approveUser: (userId: string) =>
    patch<ApiResponse<{ message: string; user: AdminUser }>>("/admin/users/approve", {
      userId,
    }),
  suspendUser: (userId: string, reason?: string) =>
    patch<ApiResponse<{ message: string }>>("/admin/users/suspend", { userId, reason }),
  reactivateUser: (userId: string) =>
    patch<ApiResponse<{ message: string }>>("/admin/users/reactivate", { userId }),
  updateUserRole: (userId: string, role: "ADMIN" | "ANALYST" | "VIEWER") =>
    patch<ApiResponse<{ message: string; user: AdminUser }>>("/admin/users/role", {
      userId,
      role,
    }),
  deleteUser: (userId: string) =>
    del<ApiResponse<{ message: string }>>(`/admin/users/${userId}`),
};
...
// ─── Direct API Client Export ─────────────────────────────────────────────────
// For scenarios where you need raw access to the request function
